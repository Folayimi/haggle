...
avatar_url TEXT,
role VARCHAR(20) DEFAULT 'buyer', -- buyer/seller/both
verification_status VARCHAR(20), -- unverified/verified/business
rating DECIMAL(3,2),
total_haggles INTEGER DEFAULT 0,
successful_haggles INTEGER DEFAULT 0,
created_at TIMESTAMP DEFAULT NOW(),
last_active TIMESTAMP
);

Haggle Sessions Table
...
instant_buy_price DECIMAL(10,2),
session_type VARCHAR(20), -- flash/standard/premier
room_atmosphere VARCHAR(30),
...
CREATE TABLE offers (
id UUID PRIMARY KEY,
session_id UUID REFERENCES haggle_sessions(id),
buyer_id UUID REFERENCES users(id),
amount DECIMAL(10,2),
status VARCHAR(20), -- pending/accepted/rejected/countered
response_time INTEGER, -- seconds to respond
offer_number INTEGER, -- sequence in negotiation
parent_offer_id UUID REFERENCES offers(id), -- for counteroffer chains
created_at TIMESTAMP DEFAULT NOW()
);

Negotiation Events Table

CREATE TABLE negotiation_events (
id UUID PRIMARY KEY,
session_id UUID REFERENCES haggle_sessions(id),
user_id UUID REFERENCES users(id),
event_type VARCHAR(30), -- offer/counter/accept/reject/join/leave/speak
metadata JSONB,
created_at TIMESTAMP DEFAULT NOW()
);
